...
def open_align_fire_rasters(
    ...
    # Open fire severity
    sev_rxr = rxr.open_rasterio(file_paths['BASELAYERS']['severity'], chunks='auto')

    # Ag/dev mask: open and clip to bbox
    agdev_mask_rxr = rxr.open_rasterio(file_paths['BASELAYERS']['agdev_mask'], chunks='auto')
    ...
    last_pre_fire_date = np.datetime64(str(fire_metadata['FIRE_DATE'].astype(object).year - 1)+'-01-01')
    first_post_fire_date = np.datetime64(str(fire_metadata['FIRE_DATE'].astype(object).year + 1)+'-01-01') # need to add a year, because annual_disturbance date is set to Dec 31 of that year
    
    future_dist_rxr = dist_rxr.sel(
        time=slice(
    ...
    grouping_rxr = grouping_rxr \
        ...
        .sortby("time").isel(time=-1) # Only select the latest date pre-fire    
    
    # Reproject, align all to same grid
    _ , sev_rxr, cuml_dist_rxr, past_dist_rxr, future_dist_rxr, ag_dev_mask, grouping_rxr = reproj_align_rasters('reproj_match', target_raster, sev_rxr, cuml_dist_rxr, past_dist_rxr, future_dist_rxr, ag_dev_mask, grouping_rxr)
    ...
    return sev_rxr, cuml_dist_rxr, past_dist_rxr, future_dist_rxr, ag_dev_mask, grouping_rxr
...
def create_ndvi_timeseries_rxr(
    ...
    for f in seasonal_ndvi_paths:
        ...
        yr, month = int(f_name[:4]), int(config['RECOVERY_PARAMS']['MONTH_SEASON_DICT'][int(f_name[4:])])
...
def create_ndvi_match_layer(
    ...
    pre_fire_end_date = fire_metadata['FIRE_DATE']
    ...
    ndvi_vals_prefire = fire_datacube['NDVI'].sel(
    ...
    updated_groups = np.full(grouping_rxr.data.shape, nodata, dtype=out_dtype)   # initialize new groupings with nodata
    for base_group in base_groupings:
        ...
        if len(group_med_ndvi_arr)>0: # if no prefire ndvi available for this group, skip
            ...
            if config['RECOVERY_PARAMS']['MAKE_PLOTS']: 
                ...
                veg_name = groupings_df['NLCD_NAME'][groupings_df['id']==base_group]
                elevation = groupings_df['ELEV_LOWER_BOUND'][groupings_df['id']==base_group]
...
def create_fire_datacube(
    ...
    sev_rxr, cuml_dist_rxr, past_dist_rxr, future_dist_rxr, ag_dev_mask, grouping_rxr = open_align_fire_rasters(
        target_raster,
        config,
        fire_metadata,
        file_paths
